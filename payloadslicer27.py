def main():
    payload = raw_input("Input your payload here: ")

    try:
        vrble = raw_input("Enter the variable name [Str is Default]: ")
    ...
    try:
        padding = raw_input("Enter the padding number [50 is default]: ")
    ...
    for i in range(0, len(payload), n):
        print('%s = %s + "%s"' % (variable, variable, payload[i:i+n]))
